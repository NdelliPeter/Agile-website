...
import mapData from "@/app/lib/africa-map.json";
...
export function AfricaMap({ className = "" }: { className?: string }) {
  ...
  const cx = labels.reduce((s, l) => s + l.x, 0) / Math.max(labels.length, 1);
  const cy = labels.reduce((s, l) => s + l.y, 0) / Math.max(labels.length, 1);
    ...
    <div className={`relative ${className}`}>
      ...
      >
        ...
        <circle cx={cx} cy={cy} r={170} fill="url(#cemacGlow)">
          <animate attributeName="r" values="160;195;160" dur="4.5s" repeatCount="indefinite" />
          <animate
      ...
      <div
        className="pointer-events-none absolute bottom-3 right-3 min-w-[180px] max-w-[240px] rounded-md border border-border bg-background/95 px-4 py-3 text-left shadow-lg backdrop-blur transition-opacity duration-200"
      ...
      >
        ...
        {hover && CEMAC_FACTS[hover.iso] && (
          <>
            ...
            <div className="text-[11px] text-muted-foreground">{CEMAC_FACTS[hover.iso].sector}</div>
          </>
        )}
      </div>
    </div>
