  ...
  return (
    <div className="relative w-full">
      ...
      <ol className="md:hidden">
        {steps.map((s, i) => (
          <li key={i} className="relative grid grid-cols-[36px_1fr] gap-5 pb-10 last:pb-0">
            <span
              aria-hidden
          ...
          {steps.map((s, i) => (
            <Fragment key={i}>
              <li className="group relative">
                <span className="relative z-10 flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background font-display text-[18px] font-bold tracking-[0.14em] text-primary transition-colors group-hover:border-primary group-hover:bg-primary group-hover:text-primary-foreground">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <div className="mt-6">
      ...
      {steps.map((s, i) => (
        <li key={i} className="relative grid grid-cols-[28px_1fr] gap-4 pb-8 last:pb-0">
          ...
          <div>
            ...
            {s.detail && (
              <p className="mt-1.5 text-[16px] leading-relaxed text-muted-foreground">{s.detail}</p>
            )}
          </div>
        </li>
      ))}
